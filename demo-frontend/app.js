...
// Presets de notificaciones tipo Duolingo
const PRESETS = {
    streak: {
        type: 'streak',
    ...
    lesson: {
        ...
        message: 'Te quedan 3 lecciones para completar el nivel. ¡Continúa aprendiendo!',
        priority: 3,
        ttl: 300
    },
    challenge: {
...
// Auto-cargar notificaciones al inicio
document.addEventListener('DOMContentLoaded', () => {
    loadNotifications();
    updateFormByType(); // Cargar preset inicial
...
function displayStream(stream) {
    ...
    if (!stream || stream.length === 0) {
        list.innerHTML = '<div style="text-align: center; color: var(--text-secondary); padding: 20px;">Sin historial</div>';
        return;
    }
    
    list.innerHTML = stream.slice(-10).reverse().map(entry => {
        const values = entry.values || entry.Values;
        return `
            <div class="stream-item">
                <div><strong>${values.title}</strong></div>
                <div style="font-size: 12px; color: var(--text-secondary);">${values.message}</div>
                <div class="stream-id">Stream ID: ${entry.streamId || entry.StreamId}</div>
            </div>
        `;
    }).join('');
}
...
async function flushRedis() {
    if (!confirm('⚠️ ¿Estás seguro de que deseas eliminar todas las notificaciones de Redis?')) {
        return;
    }
